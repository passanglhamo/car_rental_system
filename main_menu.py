...
def route_user(user, car_service,user_service,booking_service,auth_service,loyalty_service):
    if user.role == role.SYSTEM_ADMIN.value:
        system_admin_menu(user,user_service)
    elif user.role == role.ADMIN.value:
        admin_menu(user, car_service,booking_service,user_service)
    elif user.role == role.CUSTOMER.value:
        customer_menu(user, car_service,auth_service,user_service, booking_service,loyalty_service)
    else:
        print(f"Unknown role: {user.role.value}")

def main_menu(user,car_service, auth_service,user_service,booking_service,loyalty_service):
    while True:
        display_car_list(car_service.get_cars_by_limit(5), "Latest Cars")
        ...
        if entry_choice == "1":
            ...
            if user:
                route_user(user, car_service,user_service,booking_service,auth_service,loyalty_service)
